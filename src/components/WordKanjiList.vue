...
const kanjiInWord = computed<Set<string>>(() => {
  ...
  if (!props.word.writings) return kanji;
  for (const writing of props.word.writings) {
    if (writing.irregular || writing.outdated || writing.searchOnly) continue;
    for (const char of writing.text) if (isKanji(char)) kanji.add(char);
  }
  return kanji;
});
</script>
...
<template>
  <section v-if="kanjiInWord.size > 0">
    <h3>Characters ({{ kanjiInWord.size }})</h3>
    <ul class="kanji-list">
...
<style scoped>
.kanji-list {
  display: grid; gap: 1.5em 1em; grid-template-columns: repeat(3, 1fr);
